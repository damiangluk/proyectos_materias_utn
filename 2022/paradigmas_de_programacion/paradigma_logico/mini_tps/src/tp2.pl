...
incompatibles(Tipo1,Tipo2) :-  tipo(_, Tipo1), tipo(_, Tipo2), noCompartenEspecie(Tipo1, Tipo2).

noCompartenEspecie(Tipo1, Tipo2) :- forall((tipo(Especie, Tipo1), tipo(Especie, Tipo)), Tipo \= Tipo2).

ultimaEvolucion(Especie) :- evolucion(_, Especie), not(evolucion(Especie, _)).

predecible(Especie) :- tipo(Especie, Tipo), forall(puedeEvolucionar(Especie, Evolucion), tipo(Evolucion, Tipo)).
