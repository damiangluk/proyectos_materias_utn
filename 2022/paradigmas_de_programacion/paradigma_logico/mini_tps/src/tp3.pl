...
lecturaIntensa(libro(_, _, CantPaginas)) :- CantPaginas >= 500.
lecturaIntensa(libro(_, paidos, _)).
lecturaIntensa(paper(_, CantPaginas, CantVisitas)) :- (CantPaginas - CantVisitas) > 100.
lecturaIntensa(saga(_, CantLibros)) :- CantLibros > 3.

lectorCasual(Persona) :- leyo(Persona, _), not((leyo(Persona, Material), lecturaIntensa(Material))).

lecturaMasLarga(Persona, Material) :- leyo(Persona, Material), forall((leyo(Persona, UnMaterial), Material \= UnMaterial), esMasLargaQue(Material, UnMaterial)).

esMasLargaQue(saga(_, CantLibros1), saga(_, CantLibros2)) :- CantLibros1 > CantLibros2.
esMasLargaQue(saga(_, _), libro(_, _, _)).
esMasLargaQue(saga(_, _), paper(_, _, _)).
esMasLargaQue(Material1, Material2) :- tienePaginas(Material1, CantPaginas1), tienePaginas(Material2, CantPaginas2), CantPaginas1 > CantPaginas2.
esMasLargaQue(Material, Haiku) :- esHaiku(Haiku), not(esHaiku(Material)).

tienePaginas(libro(_, _, CantPaginas), CantPaginas).
tienePaginas(paper(_, CantPaginas, _), CantPaginas).
