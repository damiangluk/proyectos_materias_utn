...
votosTotales(Partido, Votos) :- 
	votos(Partido, _, _), 
	findall(UnosVotos, votos(Partido, _, UnosVotos), TodosLosVotos), 
	sum_list(TodosLosVotos, Votos).

sacoMuchosVotos(Partido, Provincia) :- 
	votos(Partido, Provincia, Votos), 
	padron(Provincia, Empadronados),
	(Votos / Empadronados) * 100 > 20.

repartida(Provincia) :- 
	votos(_, Provincia, _), 
	findall(Partido, (votos(Partido, Provincia, _), sacoMuchosVotos(Partido, Provincia)), Partidos), 
	list_to_set(Partidos, PartidosSinRepetir),
	length(PartidosSinRepetir, Cantidad), Cantidad >= 3.

pocofollower(Partido) :- votos(Partido, _, _), not(sacoMuchosVotos(Partido, Provincia)).
